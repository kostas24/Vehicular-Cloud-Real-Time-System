...
public class ControllerRequestDashboard implements ActionListener {
	...
	static String password = "Rudyoscar182";
	...
	private void acceptVehicleOwnerWriteToFile(VehicleOwner Owner) {
		...
		String phone = Integer.toString(Owner.getPhoneNum());
		String licensePlate = Owner.getVehicleList().get(0).getLicensePlate();
		String residency = Integer.toString(Owner.getVehicleList().get(0).getResidencyTime());
		String carMake = Owner.getVehicleList().get(0).getCarMake();
		String carModel = Owner.getVehicleList().get(0).getCarModel();
		String carYear = Integer.toString(Owner.getVehicleList().get(0).getCarYear());

		try {
		connection = DriverManager.getConnection(url, username, password);
		PreparedStatement statement = connection.prepareStatement("INSERT INTO vcrtsowners(Name,Email,ID,PhoneNumber,LicensePlate,ResidencyTime,CarMake,CarModel,CarYear) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
		...
		statement.setString(6, residency);
		statement.setString(7, carMake);
		statement.setString(8, carModel);
		statement.setString(9, carYear); 
	...
	private void acceptVehicleRenterWriteToFile(VehicleRenter Renter) {
		...
		String id = Renter.getId();
		String phone = Integer.toString(Renter.getPhoneNum());
		String jobDuration = Integer.toString(((Job) Renter.getJobList().get(0)).getJobDuration());
		String jobID = Integer.toString(((Job) Renter.getJobList().get(0)).getJobID());
		
		try {
			connection = DriverManager.getConnection(url, username, password);
			PreparedStatement statement = connection.prepareStatement("INSERT INTO vcrtsrenters(Name, Email, ID, Phone, JobDuration, JobID) VALUES (?, ?, ?, ?, ?, ?)");
			
			statement.setString(1, name);
			statement.setString(2, email);
			statement.setString(3, id);
			statement.setString(4, phone);
			statement.setString(5, jobDuration);
